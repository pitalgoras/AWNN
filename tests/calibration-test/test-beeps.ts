...
function generateBeepSignal(sr: number, amplitude: number): {
  signal: Float32Array;
...
export async function runBeepTest(
  ctx: AudioContext,
  ...
  const { signal, expectedEdgeSamples, totalSamples } = generateBeepSignal(sr, amp);
  ...
  return new Promise((resolve) => {
    const timeout = setTimeout(() => {
      ...
      resolve({
        ...
        detectedEdges: [], expectedEdges: expectedEdgeSamples, matchedExpectedSamples: [], matchedDetectedSamples: [],
      });
    }, 15000);
      ...
      if (e.data.type === 'RESULT') {
        ...
        if (outliers.length >= matchedDetected.length / 2) {
          resolve({
            ...
            detectedEdges, expectedEdges: expectedEdgeSamples, matchedExpectedSamples: matchedExpected, matchedDetectedSamples: matchedDetected,
          });
          return;
        }
