  ...
  persist(
    (set, get) => ({
      ...
      addTrack: (track) => set((state) => ({ 
        tracks: [...state.tracks, { ...track, id: Math.random().toString(36).substr(2, 9) }] 
      })),
      ...
      addPhrase: (trackId, phrase) => set((state) => {
        ...
        const takeNumber = (track?.phrases.length || 0) + 1;
        const newPhrase = { 
          ...phrase, 
        ...
        return {
          tracks: state.tracks.map(t => t.id === trackId ? { ...t, phrases: [...t.phrases, newPhrase] } : t)
        };
      }),
      updatePhrase: (trackId, phraseId, updates) => set((state) => ({
        tracks: state.tracks.map(t => t.id === trackId ? {
          ...t,
          phrases: t.phrases.map(p => p.id === phraseId ? { ...p, ...updates } : p)
        } : t)
      })),
      updatePhrasePosition: (trackId, phraseId, position) => set((state) => {
        ...
        if (Math.abs(delta) < 0.0001) return state;
        return {
          tracks: state.tracks.map(t => t.id === trackId ? {
            ...t,
            phrases: t.phrases.map(p => p.id === phraseId ? { ...p, startPosition: newPosition } : p),
            envelope: t.envelope.map(node => {
              if (node.time >= oldPosition - 0.01 && node.time <= oldPosition + phrase.duration + 0.01) {
                return { ...node, time: Math.max(0, node.time + delta) };
              }
              return node;
            })
          } : t)
      ...
      shiftAllPhrases: (amount, exceptPhraseId) => set((state) => ({
        tracks: state.tracks.map(t => ({
          ...t,
          phrases: t.phrases.map(p => p.id === exceptPhraseId ? p : { ...p, startPosition: Math.max(0, p.startPosition + amount) })
        }))
      })),
      removePhrase: (trackId, phraseId) => set((state) => ({
        tracks: state.tracks.map(t => t.id === trackId ? {
          ...t,
          phrases: t.phrases.filter(p => p.id !== phraseId)
        } : t)
      })),
      addCue: (cue) => set((state) => ({
        cues: [...state.cues, { ...cue, id: Math.random().toString(36).substr(2, 9) }].sort((a, b) => a.time - b.time)
      })),
      updateCue: (id, updates) => set((state) => ({
        cues: state.cues.map(c => c.id === id ? { ...c, ...updates } : c).sort((a, b) => a.time - b.time)
      })),
      ...
      addEnvelopeNode: (trackId, node) => set((state) => ({
        tracks: state.tracks.map(t => t.id === trackId ? {
          ...t,
          envelope: [...t.envelope, { ...node, id: node.id || Math.random().toString(36).substr(2, 9) } as EnvelopeNode].sort((a, b) => a.time - b.time)
        } : t)
      })),
      updateEnvelopeNode: (trackId, nodeId, updates) => set((state) => ({
        tracks: state.tracks.map(t => t.id === trackId ? {
          ...t,
          envelope: t.envelope.map(n => n.id === nodeId ? { ...n, ...updates } : n).sort((a, b) => a.time - b.time)
        } : t)
      })),
      removeEnvelopeNode: (trackId, nodeId) => set((state) => ({
        tracks: state.tracks.map(t => t.id === trackId ? {
          ...t,
          envelope: t.envelope.filter(n => n.id !== nodeId)
        } : t)
      })),
      setBpm: (bpm) => set({ bpm }),
          ...
          if (projectData) {
            const tracksWithUrls = projectData.tracks.map((t: Track) => {
              const updatedPhrases = t.phrases?.map((p, index) => {
                ...
                return { ...p, name: p.name || `Take ${index + 1}` };
              }) || [];
            ...
            set({
              ...
              globalLatencyMs: projectData.globalLatencyMs || 0,
            });
