...
class MultiTrack extends EventEmitter<MultitrackEvents> {
  ...
  private updatePosition(time: number, autoCenter = false, isInternal = false) {
    ...
    // If this is a manual seek (not from the internal sync loop) and we are playing,
    // we need to update the sync base times to avoid "jumping back"
    if (!isInternal && this._isPlaying) {
      this.startAudioTime = this.audioContext.currentTime
      this.startMultitrackTime = time
    }
  ...
  private startSync(startAt: number) {
    ...
    this.startMultitrackTime = this.currentTime
    ...
    const onFrame = () => {
      ...
      // Subtract outputLatency to show what the user is actually hearing
      const outputLatency = (this.audioContext as AudioContext & { outputLatency?: number }).outputLatency || 0
      const elapsed = Math.max(0, now - this.startAudioTime - outputLatency)
      const position = this.startMultitrackTime + elapsed * this.getAudioRate()

      if (Math.abs(position - this.currentTime) > 0.001) {
  ...
  /** Set time in seconds */
  public setTime(time: number, autoCenter = false) {
    this.updatePosition(time, autoCenter)
  }
