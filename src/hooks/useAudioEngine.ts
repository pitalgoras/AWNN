...
WaveSurfer.prototype.load = function (this: WaveSurfer, ...args: unknown[]) {
  const promise = (originalLoad as (...args: unknown[]) => Promise<void>).apply(
    this,
    args
  );
...
WaveSurfer.prototype.play = function (this: WaveSurfer, ...args: unknown[]) {
  const promise = (originalPlay as (...args: unknown[]) => Promise<void>).apply(
    this,
    args
  );
  if (promise && typeof promise.catch === "function") {
    promise.catch((e: Error) => {
      if (e && e.name === "AbortError") return;
      if (e && e.message && e.message.includes("aborted by the user agent"))
        return;
    ...
    for (let beat = 0; beat < beatsPerBar; beat++) {
      ...
      for (let i = 0; i < clickSamples; i++) {
        if (sampleIndex + i < data.length) {
          ...
          data[sampleIndex + i] +=
            Math.sin(2 * Math.PI * freq * (i / sampleRate)) * envelope * 0.5;
        }
      }
    }
  }
...
export function useAudioEngine() {
  ...
  const metronomeBufferRef = useRef<{
    bpm: number;
    timeSignature: [number, number];
    buffer: AudioBuffer;
  } | null>(null);
  ...
  const layoutRetryRef = useRef(0); // safeguard for init stuck
  const adjustLayout = useCallback(() => {
    ...
    currentTracks.forEach((t) => {
      ...
      const isExpanded =
        t.id === selectedTrackId && !envelopeLocked && !isMetronome;
      totalHeight += isMetronome
        ? currentMetronomeHeight
        : isExpanded
        ? expandedHeight
        : normalHeight;
    });
    ...
    const multitrackItems =
      (multitrackRef.current as { tracks: TrackOptions[] }).tracks || [];

    // Check if DOM elements are rendered
    const trackElements = document.querySelectorAll(
      ".multitrack-container > div > div > div"
    );
    ...
    if (multitrackItems.length === 0 || trackElements.length === 0) {
      if (layoutRetryCountRef.current < 30) {
  ...
  const trackStructureHash = useMemo(() => {
    ...
    return (
      (tracks || [])
        ...
        .join(",") +
      `|bpm:${bpm}|ts:${timeSignature?.[0]}/${timeSignature?.[1]}`
    );
      ...
      if (t.phrases.length === 0) {
        multitrackItems.push({
          ...
          options: {
            ...
            height: isMetronome
              ? metronomeHeight
              : t.id === selectedTrackId && !envelopeLocked
              ? Math.floor(trackHeight * 1.5)
              : trackHeight,
            pixelRatio,
            sampleRate,
        ...
        t.phrases.forEach((p) => {
          ...
          const realStartPosition = isMetronome
            ? p.startPosition
            : p.startPosition + secondsPerBar;
  ...
  useEffect(() => {
    ...
    multitrack.on(
      ...
      (event: { id: string; startPosition: number }) => {
        ...
        for (const track of state.tracks) {
          if (track.phrases.some((p) => p.id === id)) {
            ...
            if (
              phrase &&
              Math.abs(phrase.startPosition - startPosition) > 0.001
            ) {
  ...
  useEffect(() => {
    ...
    const pollTime = () => {
      ...
      if (
        ...
        typeof multitrackRef.current.getCurrentTime === "function"
      ) {
        try {
          ...
          const clampedUserTime = isPreRolling
            ? userTime
            : Math.max(0, userTime);
          ...
          const now = Date.now();
          if (
            typeof clampedUserTime === "number" &&
            !isNaN(clampedUserTime) &&
            (Math.abs(clampedUserTime - stateTime) > 0.1 ||
              (state.isPlaying && now - lastTimeUpdateRef.current > 100))
          ) {
          ...
          if (
            ...
            if (
              multitrackRef.current &&
              typeof multitrackRef.current.pause === "function"
            ) {
          ...
          currentTracks.forEach((track) => {
            ...
            if (track.envelope && track.envelope.length > 0) {
              ...
              if (userTime <= nodes[0].time) {
                ...
                for (let i = 0; i < nodes.length - 1; i++) {
                  if (
                    userTime >= nodes[i].time &&
                    userTime <= nodes[i + 1].time
                  ) {
            ...
            for (let j = 0; j < count; j++) {
              ...
              if (ws && typeof ws.setVolume === "function") {
                ...
                if (
                  lastVol === undefined ||
                  Math.abs(lastVol - targetVolume) > 0.001
                ) {
    ...
    return () => {
      ...
      try {
        if (multitrackRef.current) {
          const mt = multitrackRef.current as MultiTrack & {
            _layoutObserver?: { disconnect: () => void };
          };
  ...
  useEffect(() => {
    ...
    (tracks || []).forEach((track) => {
      ...
      const isExpanded =
        track.id === selectedTrackId && !envelopeLocked && !isMetronome;
      const h = isMetronome
        ? currentMetronomeHeight
        : isExpanded
        ? expandedHeight
    ...
    if (multitrackRef.current.tracks) {
      multitrackRef.current.tracks.forEach((track: TrackOptions, i: number) => {
        ...
        if (
          ...
          !String(track.id).startsWith("empty_")
        ) {
          track.draggable = !moveLocked;

          // Update cursor visually
          if (
            multitrackRef.current.rendering &&
            multitrackRef.current.rendering.containers
          ) {
      ...
      if (item && !String(item.id).startsWith("empty_")) {
        ...
        if (phrase) {
          try {
            ...
            try {
              regionsPlugin = (ws as { getActivePlugins?: () => unknown[] })
                .getActivePlugins?.()
                ?.find(
                  (p: unknown) =>
                    p &&
                    typeof (p as { addRegion?: unknown }).addRegion ===
                      "function"
                );
            } catch {
              // ignore
            ...
            if (regionsPlugin) {
              ...
              if (regions.length === 0) {
                ...
                region.setOptions({
                  color: isSelected
                    ? "rgba(79, 70, 229, 0.2)"
                    : "rgba(0, 0, 0, 0)", // Light indigo overlay when selected
                });
              }
            }
          } catch (err) {
  ...
  const startPositionsHash = useMemo(() => {
    return (tracks || [])
      .map((t) => (t.phrases || []).map((p) => p.startPosition).join(","))
      .join("|");
  }, [tracks]);
  useEffect(() => {
    if (!multitrackRef.current || !isReady) return;
    ...
    (tracks || []).forEach((track) => {
      if (track.phrases.length === 0) {
        ...
        track.phrases.forEach((phrase) => {
          if (
            typeof multitrackRef.current?.setTrackStartPosition === "function"
          ) {
            ...
            try {
              ...
              multitrackRef.current.setTrackStartPosition(
                itemIndex,
    ...
    const timeoutId = setTimeout(() => {
      try {
        if (multitrackRef.current) {
          ...
          multitrackRef.current.setMainWidth(
            [],
            multitrackRef.current.maxDuration,
            multitrackRef.current.options
          );
  ...
  const [debouncedTimeSignature, setDebouncedTimeSignature] =
  ...
  useEffect(() => {
    ...
    const metronomePhrases =
      (tracks || []).find((t) => t.id === "metronome")?.phrases || [];

    // Check if we need a completely new set of phrases (BPM or time signature changed)
    const needsNewPhrases =
      metronomePhrases.length === 0 ||
    ...
    if (
      !needsNewPhrases &&
      metronomePhrases[0] &&
      metronomePhrases[0].duration >= finalTargetDuration
    )
      return;
  ...
  const playPause = useCallback(async () => {
    ...
    if (useStore.getState().isRecording) {
      ...
      if (
        ...
        continuousRecorderRef.current.state !== "inactive"
      ) {
      ...
      if (
        ...
        const runningTime =
          (performance.now() - recorderStartTimeRef.current) / 1000;
        if (runningTime > 5) {
          continuousRecorderRef.current.stop();
        }
      }
  ...
  const seekTo = useCallback(
    (userTime: number, allowPreRoll = false) => {
      if (!multitrackRef.current) return;
  ...
  const startContinuousRecorder = useCallback(() => {
    ...
    mediaRecorder.onstop = async () => {
      ...
      const punchInOffsetSec =
        (punchInTimeRef.current - recorderStartTimeRef.current) / 1000;
      ...
      if (expectedPreRoll > 0) {
        ...
        actualTrimSec =
          punchInOffsetSec + Math.max(0, expectedPreRoll - keepPreRollSec);
        startPos =
          recordingStartTransportTimeRef.current +
          Math.max(0, expectedPreRoll - keepPreRollSec) -
          latencySec;
      } else {
        ...
        startPos =
          recordingStartTransportTimeRef.current -
          (punchInOffsetSec - actualTrimSec) -
          latencySec;
      }
  ...
  const startRecording = useCallback(
    async (trackId: string) => {
      try {
        ...
        const recordStartUserTime =
          effectivePreRollMode !== "none"
            ? punchInUserTime - secondsPerBar
            : punchInUserTime;
        ...
        expectedPreRollRef.current =
          effectivePreRollMode !== "none" ? secondsPerBar : 0;
        ...
        if (multitrackRef.current) {
          multitrackRef.current.setRecordingMode(
            effectivePreRollMode !== "none"
          );
        }
        ...
        expectedPreRollRef.current =
          (effectivePreRollMode !== "none" ? secondsPerBar : 0) + startDelay;

        // 4. Start Playback
        if (
          multitrackRef.current &&
          typeof multitrackRef.current.play === "function"
        ) {
          if (!isCurrentlyPlaying) {
