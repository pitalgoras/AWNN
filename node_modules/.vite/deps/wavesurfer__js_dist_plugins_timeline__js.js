...
var r = class _r extends e {
  constructor(t2) {
    super(t2 || {}), this.notchElements = /* @__PURE__ */ new Map(), this.currentTimeline = null, this.options = Object.assign({}, n, t2), this.timelineWrapper = this.initTimelineWrapper();
  }
  static create(t2) {
    return new _r(t2);
  }
  onInit() {
    ...
    this.options.insertPosition ? (e2.firstElementChild || e2).insertAdjacentElement(this.options.insertPosition, this.timelineWrapper) : e2.appendChild(this.timelineWrapper);
    const i2 = this.wavesurfer.getState();
    this.subscriptions.push(function(t3, e3) {
      let i3;
      const s2 = () => {
        i3 && (i3(), i3 = void 0), i3 = t3();
      }, n2 = e3.map((t4) => t4.subscribe(s2));
      return s2(), () => {
        i3 && (i3(), i3 = void 0), n2.forEach((t4) => t4());
      };
    }(() => {
      (i2.duration.value > 0 || this.options.duration) && this.initTimeline();
    }, [i2.duration])), this.subscriptions.push(this.wavesurfer.on("redraw", () => this.initTimeline())), this.subscriptions.push(this.wavesurfer.on("scroll", (t3, e3, i3, s2) => {
      this.currentTimeline && this.updateVisibleNotches(i3, s2, this.currentTimeline);
    })), ((null === (t2 = this.wavesurfer) || void 0 === t2 ? void 0 : t2.getDuration()) || this.options.duration) && this.initTimeline();
  }
  destroy() {
  ...
  initTimeline() {
    ...
    const b = s("div", { style: { width: "0", height: "50%", display: "flex", flexDirection: "column", justifyContent: v ? "flex-start" : "flex-end", top: v ? "0" : "auto", bottom: v ? "auto" : "0", overflow: "visible", borderLeft: "1px solid currentColor", opacity: `${null !== (a = this.options.secondaryLabelOpacity) && void 0 !== a ? a : 0.25}`, position: "absolute", zIndex: "1" } });
    for (let t3 = 0, e3 = 0; t3 < h; t3 += u, e3++) {
      const i3 = b.cloneNode(), s2 = Math.round(100 * t3) % Math.round(100 * c) == 0 || d && e3 % d == 0, n3 = Math.round(100 * t3) % Math.round(100 * f) == 0 || m && e3 % m == 0;
      ...
      const o2 = (t3 + this.options.timeOffset) * p;
