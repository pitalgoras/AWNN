...
- [x] **Latency Compensation (Data-Level):** 
  ...
  - **Raw Recording Mode:** Option to disable audio filtering during recording. Chrome uses `{ exact: false }` constraints + `goog*` WebRTC flags. Firefox uses `echoCancellation: false` alone to trigger WebRTC AudioProcessing passthrough mode (disables all processing as a side effect). See devhistory.md §24 for details.
