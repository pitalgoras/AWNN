...
5. **Latency Compensation:** Includes a Settings UI with a manual input field for the latency value and an "Auto-Calibrate" tool that uses a dedicated AudioWorklet (`calibration.worklet.js`) for sample-accurate rising edge detection. The calibrator plays a 500ms sustain tone (1kHz, 70% amplitude) through speakers to wake Bluetooth devices, followed by 5 peaks at non-regular intervals `[100, 140, 100, 100, 140]`ms derived from bit pattern `[0,1,0,0,1]`. Edge frames from the worklet are pattern-matched against the expected intervals (±25ms tolerance) to compute round-trip latency. Uses same raw audio constraints as the recording engine. The visual playhead position is also compensated by the browser's `outputLatency` to ensure it matches the audible sound. Each track also supports a manual `offset` for fine-grained timing adjustments.
