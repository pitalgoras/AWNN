...
5. **Latency Compensation:** Includes a Settings UI with a manual input field for the latency value and an "Auto-Calibrate" tool that plays a click and records it to automatically calculate the system latency.
...
### Overview
The Song Builder is a separate mode within the existing audio editor, toggled via a header icon. It enables users to map voice parts to lyrics segments and align them with recorded audio, producing a rehearsal‑ready snapshot.

### Data Model Extensions
The `ProjectData` type in `useStore.ts` will be extended with lyrics, voicing segments, and alignment blocks.

### Track‑to‑Voice Mapping
Fixed mapping by track index (customizable via existing Track Settings).

### Painting Interaction
Drag across words to assign colors; snap to word boundaries; overwrite on repaint. `[voice]` tags remain editable.

### Alignment Engine
Auto‑alignment with adaptive onset detection; manual via drag and tap‑to‑cue.

### Publishing
Opus‑only export using File System Access API with `browser-fs-access` fallback. Manifest `songs_index.json` updated optimistically. Folder: `song_title/v1/`.

### Rehearsal Player
Loads manifest from `./songs_index.json`. Client‑side Opus decoding for Safari. Focus Mode with ducking.
